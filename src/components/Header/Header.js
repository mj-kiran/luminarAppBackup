import React, { useEffect, useState } from "react";
import "./Header.css";
import {   Nav, NavDropdown, Navbar } from "react-bootstrap";
import NotificationsIcon from "@mui/icons-material/Notifications";
import ViewCarouselIcon from "@mui/icons-material/ViewCarousel";
import {  useLocation, useNavigate } from "react-router-dom";
function Header() {
  const [activePage, setActivePage] = useState("");
const location=useLocation()
  const handleLinkClick = (link) => {
    setActivePage(link);
    localStorage.setItem("activePage", link);
  }
  const navigate = useNavigate()
  useEffect(() => {
    //  setActiveLink("overview");
    if (location.pathname === "/") {
      setActivePage("courses")
     localStorage.removeItem("activeLink");
    }
    else {
      // localStorage.removeItem("activeLink");
      const savedActiveLink = localStorage.getItem("activePage");
      setActivePage(savedActiveLink || "courses");
      
    }
    
  }, []);
  return (
    <>
      <Navbar className="navbar fixed-top" bg="light" data-bs-theme="light">
        <Nav className="navbar_headings me-auto ">
          <Nav.Link
            className={
              activePage === "courses" ? "nav_button active" : "nav_button "
            }
            onClick={() => {
              handleLinkClick("courses");
              navigate("/courses");
            }}
          >
            Courses
          </Nav.Link>
          {/* <Nav.Link
            className={
              activePage === "batches" ? "nav_button active" : "nav_button"
            }
            onClick={() => {
              handleLinkClick("batches");
              navigate("/batches");
            }}
          >
            Batches
          </Nav.Link> */}
          {/* <Nav.Link
            className={
              activePage === "timetable" ? "nav_button active" : "nav_button"
            }
            onClick={() => {
              handleLinkClick("timetable");
              navigate("timetable");
            }}
          >
            Timetable
          </Nav.Link> */}
          {/* <Nav.Link
            className={
              activePage === "enquiry" ? "nav_button active" : "nav_button"
            }
            onClick={() => {
              handleLinkClick("enquiry");
              navigate("*");
            }}
          >
            Enquiry
          </Nav.Link> */}
          <Nav.Link
            className={
              activePage === "payments" ? "nav_button active" : "nav_button"
            }
            onClick={() => {
              handleLinkClick("payments");
              navigate("*");
            }}
          >
            Payments
          </Nav.Link>
          {/* <Nav.Link
            className={
              activePage === "chats" ? "nav_button active" : "nav_button"
            }
            onClick={() => {
              handleLinkClick("chats");
              navigate("*");
            }}
          >
            Chats
          </Nav.Link> */}
          <Nav.Link
            className={
              activePage === "appdownloads" ? "nav_button active" : "nav_button"
            }
            onClick={() => {
              handleLinkClick("appdownloads");
              navigate("*");
            }}
          >
            App Downloads
          </Nav.Link>
          {/* <Nav.Link
            className={
              activePage === "studymaterial"
                ? "nav_button active"
                : "nav_button"
            }
            onClick={() => {
              handleLinkClick("studymaterial");
              navigate("*");
            }}
          >
            Study Material
          </Nav.Link> */}
          {/* <Nav.Link
            className={
              activePage === "store" ? "nav_button active" : "nav_button"
            }
            onClick={() => {
              handleLinkClick("store");
              navigate("*");
            }}
          >
            Store
          </Nav.Link> */}
          {/* <Nav.Link
            className={
              activePage === "couponcode" ? "nav_button active" : "nav_button"
            }
            onClick={() => {
              handleLinkClick("couponcode");
              navigate("*");
            }}
          >
            Coupon code
          </Nav.Link> */}
        </Nav>
        <div className="icons_header">
          <div className="header_icon">
            <ViewCarouselIcon />
          </div>
          <div className="header_icon">
            <NotificationsIcon />
          </div>
        </div>
        <div className="luminar_logo_section me-2">
          <img
            className="luminar_logo"
            src="data:image/jpeg;base64,"
            alt=""
          />
        </div>
        <NavDropdown
          className="login_id me-3"
          title="Luminar Technolab"
          id="basic-nav-dropdown"
        >
          <NavDropdown.Item>Log Out</NavDropdown.Item>
          <NavDropdown.Item>...</NavDropdown.Item>
          <NavDropdown.Item>...</NavDropdown.Item>
          {/* <NavDropdown.Divider /> */}
        </NavDropdown>
      </Navbar>
    </>
  );
}

export default Header;
